...
const schema = yup
  ...
  .shape({
    ...
    role: yup.string('Formato inválido').required('Campo obligatorio')
  })
  .required()

const UsersModal = ({ hidden, setModal, values, setValues }) => {
  ...
  const onSubmit = async data => {
    ...
    const response = await fetch(`/api/users/${isUpdate ? values._id : ''}`, {
    ...
    if (success) {
      setModal(false)
      router.push('/backoffice/users', '/backoffice/usuarios')
    } else {
